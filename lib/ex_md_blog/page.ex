defmodule ExMdBlog.Page do
  ...
  @posts_path @pages_path <> "posts.html.eex"
  ...
  def render_home do
    ...
    home_html = EEx.eval_file(home_full_path)
  ...
  @doc "Renders posts page and writes HTML file on pages folder"
  @spec render_posts() :: path :: String.t()
  def render_posts do
    posts_full_path = Application.app_dir(:ex_md_blog) <> @posts_path
    posts_html = EEx.eval_file(posts_full_path, post_callouts: render_post_callouts())

    base_full_path = Application.app_dir(:ex_md_blog) <> @base_path
    full_html = EEx.eval_file(base_full_path, content: posts_html)

    # removing .eex of the original path
    output_path = String.slice(posts_full_path, 0, String.length(posts_full_path) - 4)
  ...
  @spec render_all_posts() :: list({id :: String.t(), path :: String.t()})
  def render_all_posts do
